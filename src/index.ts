...
import { glob, download, runNpmInstall, runPackageJsonScript, getNodeVersion, getSpawnOptions, createLambda, Route, BuildOptions, Config, FileFsRef, Lambda } from '@now/build-utils'
...
function validateDistDir(distDir: string, isDev: boolean | undefined, config: Config): void {
...
export async function build({ files, entrypoint, workPath, config, meta = {} }: BuildOptions): Promise<{ routes: object; output: Output }> {
	...
	const distPath = path.join(workPath, mountpoint, (config && (config.distDir as string)) || 'build')
	...
	if (entrypointName === 'package.json') {
		...
		if (!server['server.js']) throw new Error('Something went wrong with the build. Please run `npx frontity dev --production` locally to find out.')
